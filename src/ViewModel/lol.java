...
class tipos_dato { 
  ...
  static ArrayList<String> enteros_codigo=new ArrayList<>();
  static ArrayList<Integer> enteros_valores=new ArrayList<>();
  static ArrayList<Integer> forloop_index=new ArrayList<>();
  static int codigo=0;
  static ArrayList<Integer> for_start=new ArrayList<>();
  
  public static void activity_for (int x,int y,int z) {
    //x define los estados del for
    //y define el rango del for
    //z define la posicion
      
    String space ="";
    
    //Crea los espacios para el for
    if (x!=0){
      for (int f=0;f!=z;f++){ 
        space="  "+space;
      }
    }
    //escribre el string del for
    if (x!=2){
      script.add(space+"for(int i=0;i<"+y+";i++){");
      for_start.add(forloop_index.size());
      crear_ejercicio(1,0,0);
      script.add("}");
    }
    //resultado del for creado
    if (x!=1){
      for (int i=0;i!=y;i++){
